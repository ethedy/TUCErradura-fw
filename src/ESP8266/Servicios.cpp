...
*/
  ...
  ServiciosSeguridad::LoginUser(const String& user_email, const String& user_pass)
  {
    ...
    {
      result.StatusCode = ValidationErrors::Bad_Formed_User_Password;
    ...
    else
    {
      ...
      if (resultado)
      {
        if (resultado->Password == user_pass.c_str())
        {
          ...
          result.Nombre = resultado->Nombre.c_str();
          result.Rol = resultado->Rol;

          result.StatusCode = ValidationErrors::OK;
          result.Status = "OK";
        }
        else
        {
          result.StatusCode = ValidationErrors::Invalid_Credentials;
      ...
      else
      {
        ...
        result.StatusCode = ValidationErrors::User_Not_Found;
...
*/
                             ...
                             const String& user_rol)
  {
    ...
    {
      Entidades::Roles usr_rol = ParseRol(tmp_rol);

      return userRepo.AddUser(tmp_email, tmp_name, tmp_pass, usr_rol);
    }
  ...
  bool
  ServiciosDatabase::HasUsersRegistered() const
  {
    RepositorioUsuarios userRepo;
